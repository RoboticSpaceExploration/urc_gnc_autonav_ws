...
int updateTargetConfig(odrive_endpoint *endpoint, Json::Value odrive_json, string config_file)
{
    ...
    if (cfg.is_open()) {
        ...
        // check calibration, don't calibrate on startup every single time
        bool pre_calibrated = false;
        readOdriveData(endpoint, odrive_json, string("axis0.motor.config.pre_calibrated"), pre_calibrated);
        if (pre_calibrated) {
            ROS_INFO("Axis 0 already pre-calibrated. Skipping axis 0 calibration");
        }
        else {
            ROS_INFO("Calibrating axis 0...");
            if (calibrateAxis0(endpoint, odrive_json) != ODRIVE_OK) {
                ROS_ERROR("* Error calibrating axis 0!");
                return ODRIVE_ERROR;
            }
            ROS_INFO("Finished calibrating axis 0!");
        }

        // check calibration, don't calibrate on startup every single time
        readOdriveData(endpoint, odrive_json, string("axis1.motor.config.pre_calibrated"), pre_calibrated);
        if (pre_calibrated) {
            ROS_INFO("Axis 1 already pre-calibrated. Skipping axis 1 calibration");
        }
        else {
            ROS_INFO("Calibrating axis 1");
            if (calibrateAxis1(endpoint, odrive_json) != ODRIVE_OK) {
                ROS_ERROR("* Error calibrating axis 1!");
                return ODRIVE_ERROR;
            }
            ROS_INFO("Finished calibrating axis 1!");
        }


        ROS_INFO("Saving odrive configuration");
    	...
    	bool save_config = 0)
{
    ...
    ROS_INFO("ODRIVE: Setting %s config value", name.c_str());
